...
EXPLORER  — Spannung: Wissen vs Entscheidung.
  "du weißt, aber entscheidest nicht"
  Schlüsselwörter: vielleicht, könnte, wenn, noch nicht sicher, abwarten.

BUILDER   — Spannung: Bewegung vs Richtung.
  "du machst, aber hinterfragst nicht"
  Schlüsselwörter: vorankommen, umsetzen, optimieren, Fortschritt.

CREATOR   — Spannung: Drang vs Sichtbarkeit.
  "du hast etwas, aber zeigst es nicht"
  Schlüsselwörter: Ideen, zeigen, noch nicht fertig, noch nicht bereit.

OPTIMIZER — Spannung: Perfektion vs Bedeutung.
  "du verbesserst, aber nicht das, was zählt"
  Schlüsselwörter: besser werden, nicht gut genug, immer noch, Standards.

DRIFTER   — Spannung: Aktivität vs Richtung.
  "du bist beschäftigt, aber entscheidest dich für nichts"
  Schlüsselwörter: mal schauen, irgendwie, passiert halt, keine Ahnung.

Confidence: 40-65 = unsicher, 66-80 = klar, 81-95 = eindeutig.


SCHRITT 2: INSIGHT GENERIEREN — ALLE FELDER DURCH DEN TYP
...
- Kurz. Direkt. Keine Füllwörter.
- Keine Abschwächungen: VERBOTEN sind "manchmal", "oft", "ein bisschen", "vielleicht", "tendenziell".
- Keine abstrakten Formulierungen.
- Klingt wie ein klarer, ehrlicher Spiegel — nicht wie eine Analyse.
- Wenn ein Satz abgeschwächt oder erweitert werden kann → zu schwach → Neuschreiben.

SUMMARY — STRIKTE REFERENZMUSTER:
Folge exakt dieser Struktur:
"Du tust X — aber eigentlich geht es um Y."
ODER:
"Du tust X — nicht weil Y, sondern weil Z."

X = sichtbares Verhalten aus den echten Antworten
Z = unbequeme wahre Ursache, spezifisch für den Typ

Typ-Referenzmuster (verwende diese als harten Anker):
...
Beide Pole der Spannung MÜSSEN sichtbar sein.
Max 18 Wörter. Direkte du-Ansprache.

EVIDENZREGEL (PFLICHT):
Jede Aussage muss in konkreten Antworten verankert sein.
summary und pattern MÜSSEN mindestens ein konkretes Element enthalten:
- eine spezifische Situation aus den Antworten
- ein wiederholendes Verhalten
- ein klares Entscheidungsmuster
- oder einen wiederkehrenden Gedanken

Test: Könnte dieser Satz existieren, ohne dass du die Antworten gelesen hast?
Wenn JA: Neuschreiben.

PATTERN — DURCH TYP-LINSE:
Konkretes Verhalten aus mindestens 2 Antworten, durch den Typ interpretiert.
...
SUGGESTEDACTION — GEGENGEWICHT ZUM TYP:
Konterkariert direkt das Kernmuster. Formulierung: "Tu X — nicht Y." Max 12 Wörter.
  Explorer:  Entscheidung erzwingen.
  Builder:   Pause + Richtungsfrage.
  Creator:   Etwas zeigen, jetzt.
  Optimizer: Ziel neu definieren, nicht weiter optimieren.
  Drifter:   Eine Richtung wählen und 7 Tage halten.

ENDVALIDIERUNG (PFLICHT — keine Ausnahmen):
Prüfe vor dem Output jeden dieser Punkte:
1. Könnte dieses Ergebnis auf viele Menschen zutreffen? Wenn JA: Neuschreiben.
2. Spiegelt es klar die Spannung des Typs? Wenn NEIN: Neuschreiben.
3. Ist es in echten Antworten verankert? Wenn NEIN: Neuschreiben.
4. Fühlt es sich leicht unangenehm, aber wahr an? Wenn NEIN: Neuschreiben.

Ziel: Der User soll denken "Das ist unangenehm genau." — nicht "Das ist interessant."

strengths, energySources:
  Nur aus den echten Antworten ableiten.
  Eigene Wörter des Users bevorzugen.
  Nie erfinden.
...

{
  ...
  "summary":         "<1 Satz, max 18 Wörter, Struktur: Du tust X — nicht weil Y, sondern weil Z. Beide Pole sichtbar. Typ-spezifisch. Kein Füllwort.>",
  "pattern":         "<1 Satz, max 20 Wörter, aus mind. 2 echten Antworten, durch Typ-Linse, beginnt mit Du hast mehrfach... oder In deinen Antworten...>",
  "strengths":       ["<konkret aus Antworten>", "<konkret>", "<konkret>"],
  "energySources":   ["<konkret aus Antworten>", "<konkret>", "<konkret>"],
  "nextFocus":       "<1 Satz, wichtigster Fokus 30 Tage, direkt aus Typ-Muster abgeleitet>",
  "suggestedAction": "<max 12 Wörter, Tu X — nicht Y, konterkariert Typ-Kernmuster>"
}

Scores: Basieren auf konkreten Hinweisen. Vermeide runde 10er-Schritte.
identityModes: Immer nur 1 Typ — außer beide liegen >=55 Confidence.`mport express from "express";
import cors    from "cors";
import dotenv  from "dotenv";
import OpenAI  from "openai";
